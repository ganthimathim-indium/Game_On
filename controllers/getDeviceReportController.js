...
const getSessions = async (req, res) => {
  ...
  if (authorisedRoles.includes(requestdRole)) {
    ...
    const sql = 'SELECT DISTINCT UD.device_id,device_name,app_name,version_name,CU.average_value as cpu_average_usage,GU.average_value as gpu_average_usage,MU.average_value as memory_average_usage,PU.average_value as power_average_usage,RU.name as user_name,email,CU.cpu_app_usage,CU.recorded_time as cpu_usage_time, GU.avg_gpu_usage,GU.recorded_time as GPU_usage_time,MU.avg_memory_usage,MU.recorded_time as memory_usage_time,PU.avg_power_usage,PU.recorded_time as power_usage_time FROM register RU FULL JOIN report_basicinfo UD ON UD.user_id = RU.id  FULL JOIN cpu_report CU ON  UD.session_id = CU.session_id FULL JOIN gpu_usage_report GU ON  CU.session_id = GU.session_id FULL JOIN memory_report MU ON  GU.session_id = MU.session_id FULL JOIN power_usage_report PU ON  MU.session_id = PU.session_id WHERE UD.user_id = $1  AND UD.device_id = $2 AND session_id = $3';
